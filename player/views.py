...
from .models import player, team, match, match_stats
...
def getTeam(request, id):
    score_team_1 = match.objects.filter(team1_id=id).aggregate(total_sum1=Sum(Coalesce('team1Score', Value(0))))['total_sum1']
    score_team_2 = match.objects.filter(team2_id=id).aggregate(total_sum2=Sum(Coalesce('team2Score', Value(0))))['total_sum2']
